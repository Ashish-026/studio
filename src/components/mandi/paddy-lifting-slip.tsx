    ...
    <div ref={ref} className="p-8 bg-white text-black w-[800px] mx-auto min-h-[1000px] border">
      ...
      <div className="grid grid-cols-2 gap-8 mb-8 text-sm">
        <div className="space-y-2">
          ...
          <p><span className="font-bold opacity-60">DATE:</span> <span className="font-bold">{format(entry.date, 'dd MMM yyyy, hh:mm a')}</span></p>
        </div>
        <div className="space-y-2 text-right">
          ...
          {entry.isPrivateOverflow && <p className="text-destructive font-black text-[10px] uppercase">!! OVERFLOW DETECTED !!</p>}
        </div>
      </div>
      ...
      {entry.individualBagWeights && entry.individualBagWeights.length > 0 ? (
          ...
          <div className="grid grid-cols-8 gap-2">
            {entry.individualBagWeights.map((w, i) => (
              <div key={i} className="border p-1 text-center">
                <p className="text-[8px] opacity-40 font-bold leading-none mb-1">{i + 1}</p>
                <p className="text-xs font-bold">{w.toFixed(1)}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="h-20 flex items-center justify-center border-2 border-dashed border-muted rounded-xl mb-8">
          <p className="text-xs text-muted-foreground italic">No individual bag data recorded for this entry.</p>
        </div>
      )}
        ...
        <div className="grid grid-cols-2 gap-12">
          <div className="space-y-3">
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="font-bold">{(entry.totalPaddyReceived * 100).toLocaleString()} KG</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="font-bold opacity-60 uppercase tracking-tighter">Mandi Standard Wt:</span>
              <span className="font-bold">{(entry.mandiWeight * 100).toLocaleString()} KG</span>
            </div>
            {entry.privateOverflowQty && entry.privateOverflowQty > 0 ? (
              <div className="flex justify-between items-center text-xs text-destructive font-bold pt-2 border-t border-destructive/10">
                <span>PRIVATE OVERFLOW:</span>
                <span>{entry.privateOverflowQty.toFixed(2)} QTL</span>
              </div>
            ) : null}
          </div>
          <div className="text-right">
            <p className="text-[10px] font-black uppercase text-primary tracking-widest mb-1">Total Official Quantity</p>
            <p className="text-4xl font-black text-primary">{entry.mandiWeight.toFixed(2)} <span className="text-sm opacity-60">QTL</span></p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-32 grid grid-cols-2 gap-20">
        <div className="text-center border-t border-black pt-2">
          <p className="text-[10px] font-bold uppercase">Farmer Signature</p>
        </div>
        <div className="text-center border-t border-black pt-2">
          <p className="text-[10px] font-bold uppercase">For {millName}</p>
        </div>
      </div>

      <div className="mt-20 text-[8px] text-muted-foreground text-center italic uppercase">
        Computer generated document from Mandi Monitor. Valid without physical stamp.
      </div>
    </div>
  );
});
